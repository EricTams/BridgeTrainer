...
for (const [key, expectedCode] of entries) {
  ...
  wrongCases.push({
    handText, history, expectedCode, gotCode: topCode,
...
console.log(`=== Wrong answers by causing rule (${sortedRules.length} rules) ===\n`);
for (const [ruleId, cases] of sortedRules) {
  console.log(`--- ${ruleId} (${cases.length} cases) ---`);
  for (const c of cases) {
    ...
    console.log(`  "${c.history}" exp=${c.expectedCode} got=${c.gotCode} hcp=${c.hcp} shape=[${c.shape}] ${c.shapeClass} own=${ob} partner=${pb} plast=${plb}`);
  }
  console.log();
}
