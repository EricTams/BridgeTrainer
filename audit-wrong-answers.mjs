...
for(const[key,exp] of entries){
  ...
  if(got==='NONE'){norec++;continue;} if(got===exp){ok++;continue;}
  const cls=classifyAuction(auction,seat),ev=evaluate(hand);
  const pb=findPartnerBid(auction,seat),plb=findPartnerLastBid(auction,seat),ob=findOwnBid(auction,seat),opp=findOpponentBid(auction,seat);
  wrong.push({key,ht,hist,exp,got,ruleId:top.explanation.match(/\[(.*?)\]/)?.[1]||'',rule:top.explanation,
    hcp:ev.hcp,shape:ev.shape,sc:ev.shapeClass,phase:cls.phase,
    pb:pb?`${pb.level}${pb.strain===Strain.NOTRUMP?'N':pb.strain}`:'-',
    plb:plb?`${plb.level}${plb.strain===Strain.NOTRUMP?'N':plb.strain}`:'-',
    ob:ob?`${ob.level}${ob.strain===Strain.NOTRUMP?'N':ob.strain}`:'-',
    opp:opp?`${opp.level}${opp.strain===Strain.NOTRUMP?'N':opp.strain}`:'-'});
}
console.log(`Correct: ${ok} | Wrong: ${wrong.length} | No-rec: ${norec} | Total: ${entries.length}\n`);
...
for(const[id,cases] of [...byRule.entries()].sort((a,b)=>b[1].length-a[1].length)){
  ...
  for(const c of cases) console.log(`  "${c.hist}" exp=${c.exp} got=${c.got} hcp=${c.hcp} [${c.shape}] ${c.sc} own=${c.ob} p=${c.pb} pl=${c.plb} opp=${c.opp}`);
  console.log();
}
